...
fn execute_part_two(input: &Vec<i32>) -> Vec<i32> {
    for i in 0..input.len() {
        for j in 0..input.len() {
            for k in 0..input.len() {
                if (input[i] + input[j] + input[k]) == 2020i32 {
                    println!("i: {}, j: {}, k:{}, product: {}", input[i], input[j], input[k], input[i] * input[j] * input[k]);
                    return vec![input[i], input[j], input[k]];
                }
            }
        }
    }
